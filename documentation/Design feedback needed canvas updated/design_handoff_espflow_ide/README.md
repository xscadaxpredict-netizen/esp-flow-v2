...
### Slot resolution — trailing slots at a block's right edge

Each series node renders a **trailing slot** (the empty insertion cell after its last child) at
`x = right edge of that series' allotted width`. This produces a genuine collision: when a
parallel block is the last child of its parent series, the parent's trailing slot and the
block's **level-0** trailing slot land on the same x *and* the same y (level 0 shares the
parent's row). Two different insertion targets, one pixel region.

**Rule: the shallowest path wins.** A click on the slot at a block's right edge inserts
*after the block, in the parent series* — never inside the block's first level. This is what an
engineer means by that click; the alternative silently swallows the element into the branch.

Implementation: collect slot candidates during the draw pass, **sort by path depth ascending**,
then run the hit-region dedupe (first candidate to claim an x/y keeps it). Element hit regions
are registered before slots and always take precedence over both.

Appending *inside* a branch level stays reachable and must not be lost: hovering the right half
of an existing element and clicking inserts in series within the series that owns that element
(`placeAt(network, path, 'right', type)`), and the enclosing block widens automatically. So the
two intents are separated by *where* you click — element-right-half for inside, trailing slot
for after.

Levels below row 0 never collide (different y) and resolve normally.
